...
const fetchExample = (f: string): Promise<string> => fetch(`examples/${f}`).then((r) => r.text());
...
const OUTPUT_FORMATS: readonly OutputFormat[] = ["asm", "bin", "rk", "rkr", "pki", "gam"];
...
function formatDump(display: string, baseAddr: number, bytes: number[], perRow: number): string {
    const lines: string[] = [];
...
const EMULATOR_URL = (window as unknown as { asm8EmulatorUrl?: string }).asm8EmulatorUrl ?? EMULATOR_URL_DEFAULT;
