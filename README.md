...
- **upload** button loads the assembled `.rk` into the emulator _without_ running it (same handoff path as **run**, but `?load=` / `?loadoff=` instead of `?run=` / `?handoff=`). Disabled until assembly succeeds.
...
- `include "path.asm"` — inline another source file at this point. The path is resolved relative to the **including** file's directory; nested includes follow the same rule. Self- and circular-include chains are rejected. Errors inside an included file report that file's path and line. In the web playground, the quoted name instead matches an open tab exactly (case-sensitive, with no path resolution), using its current contents. The **Main** selector defaults to the active tab. Select a specific main file to keep assembling it while editing included tabs; their gutters show addresses in that assembly, and run/load/binary downloads use the main file. Source downloads still save the active tab. The selection is remembered across reloads; closing the main file returns to active-tab mode. Included fragments should omit `end`, which terminates the entire assembly. Library callers provide `AsmOptions.readInclude` to enable includes.
